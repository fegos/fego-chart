...
export default class EventCapture extends Component {
	...
	_shouldBecomeMoveResponder = (e, gestureState) => {
		...
		let should = false;
		if (eventCaptures && eventCaptures.length) {
			let touchNum = gestureState.numberActiveTouches;
	...
	_onStartShouldSetPanResponderCapture = (e, gestureState) => {
		return this.props.shouldCapturePressGesture;
	}
	...
	_onMoveShouldSetPanResponderCapture = (e, gestureState) => {
		return this.props.shouldCaptureMoveGesture;
	}
	...
	_onStartShouldSetPanResponder = (e, gestureState) => {
		return this._shouldBecomePressResponder(e, gestureState);
	}
	...
	_onMoveShouldSetPanResponder = (e, gestureState) => {
		return this._shouldBecomeMoveResponder(e, gestureState);
	}
	 ...
	 */
	_onShouldBlockNativeResponder = (e, gestureState) => {
		return this.props.shouldBlockNativeResponder;
	}
	...
	_handlePinchEvent = (eventState, e, gestureState) => {
		let { touches } = e.nativeEvent;
		if (touches && touches.length === 2) {
			let x = Math.pow(touches[0].locationX - touches[1].locationX, 2);
			let y = Math.pow(touches[0].locationY - touches[1].locationY, 2);
			let currentSpan = Math.sqrt(x + y);
			let totalScale = currentSpan / this._orignalSpan;
			let activeScale = currentSpan / this._activeSpan;
	...
	_setLongPressTimer = (e, gestureState) => {
		this._removeLongPressTimer();
		let evt = { nativeEvent: e.nativeEvent };
		let tempGestureState = Object.assign({}, gestureState);
	...
	_setDoublePressTimer = (e, gestureState) => {
		...
		let evt = { nativeEvent: e.nativeEvent };
		let tempGestureState = Object.assign({}, gestureState);
		let eventCaptures = Object.assign([], this.props.eventCaptures);
	 ...
	 * 记录双击的第一个落点
	 */
	_recordFirstTouch = (e, gestureState) => {
	 ...
	 * 判断双击第二个落点是否有效
	 */
	_isSecondTouchAvailable = (e, gestureState) => {
		if (e && e.nativeEvent) {
			if (e.nativeEvent.touches && e.nativeEvent.touches.length) {
				let firstTouch = this._firstTouch;
				let touch = e.nativeEvent.touches[0];
				let disX = Math.abs(touch.pageX - firstTouch.pageX);
				let disY = Math.abs(touch.pageY - firstTouch.pageY);
	...
	_recordOriginalSpan = (e, gestureState) => {
		...
		if (touches && touches.length === 2 && gestureState.numberActiveTouches === 2) {
			let x = Math.pow(touches[1].locationX - touches[0].locationX, 2);
			let y = Math.pow(touches[1].locationY - touches[0].locationY, 2);
