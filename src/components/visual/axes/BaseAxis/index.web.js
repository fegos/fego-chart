...
class BaseAxis extends Component {
	...
	drawAxis = (props,chartContext) => {
		let { min, max, tickNums, tickValues } = props;
		let { yScale } = chartContext;
		if(min && max && yScale){
	...
	drawFixedAxisTicks = (props, chartContext) => {
		let { axisAt, tickValues: originTickValues, scale, type, showGridLine, XAxisTickLabelOffset, tickPos } = props;
		let { containerFrame, chartFrame, context } = chartContext;
		...
		if(type === 'XAxis'){
			...
			let tickValues
			let tickObj = {}
			let { width, padding } = containerFrame
			let { origin, height:currChartHeight } = chartFrame
			tickValues = Array.from(originTickValues)
			if(tickPos === 'center'){
				tickValues.push("")
			}
			let centerPosOffset = tickPos === 'center' ? (width-padding.left-padding.right)/(2*(tickValues.length-1)) : 0
			for(let i = 0; i < tickValues.length; i++){
				let tickX = padding.left + i*((width-padding.left-padding.right)/(tickValues.length-1))
				let tickYBase = origin[1] + currChartHeight + offsets[1]
				tickObj = {
					x1: tickX ,
					y1: tickYBase,
					x2: tickX,
	...
	drawAxisTicks = (props,chartContext) => {
		...
		let { showGridLine, min, max, tickNums, tickValues } = props;
		...
		if(scale && typeof scale === 'function'){
			...
			let ticksPos = ticks.map(tick => {return this.calcTickPos(tick, props, chartContext)})
			ticksPos.map(tp => {
				this.drawEachTick(tp, props, chartContext, showGridLine)
			})
		}
	...
	calcTickPos = (tick, props, chartContext) => {
		let tickObject = {}
		let { scale, axisAt, tickLineLength, XAxisTickLabelOffset, YAxisTickLabelOffset, tickFormat, lastClose } = props;
		let { context, containerFrame, chartFrame, plotData, xScale } = chartContext
		let { axisLinePos } = this.cache;
		if(props.type === 'XAxis' && scale){
			let tickX = scale(tick)
			let currentItemIdx = getCurrentItem(xScale,null,[tickX],plotData.currentData,'index')
			let currentItem = plotData.currentData[currentItemIdx]
			...
			let textMetrics = context.measureText(tickObject.text)
			...
			if(showPercent && !isNaN(lastClose) && (+lastClose !== 0)){
				tickObject.text = (100*(tick-lastClose)/lastClose).toFixed(2)+"%"
			} else {
